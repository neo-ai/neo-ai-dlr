...
TEST_F(TVMTest, TestGetInputName) { EXPECT_EQ(model->GetInputName(0), "input_tensor");}

TEST_F(TVMTest, TestGetInputType) { EXPECT_EQ(model->GetInputType(0), "float32");}
...
TEST_F(TVMTest, TestGetInputDim) {
    EXPECT_EQ(model->GetInputDim(0), 4);
}

TEST_F(TVMTest, TestGetNumOutputs) {
...
TEST_F(TVMTest, TestGetOutputSize) {
    EXPECT_EQ(model->GetOutputSize(0), 1);
}

TEST_F(TVMTest, TestGetOutputDim) {
    EXPECT_EQ(model->GetOutputSize(0), 1);
}
...
TEST_F(TVMTest, TestTvmModelApisWithOutputMetadata) {
  ...
  EXPECT_NO_THROW(model->GetOutputByName(model->GetOutputName(0).c_str(), output2));
